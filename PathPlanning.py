...
def path_safety(obs, Px, Py):
    ...
    Reference: http://paulbourke.net/geometry/polygonmesh/
    """
    safety_value = np.zeros(Px.shape[0])
    ...
    for i in range(len(obs)):
        ...
        # Obstacle is a polygon (V = vertices, Kv = scaling factor)
            ...
            V, Kv = data[3:]
            close = np.zeros(Px.shape, dtype=bool)
            for i in range(V.shape[0]):
                a = ((V[i, 1] > Py) != (V[i-1, 1] > Py)) & \
                    (Px < (V[i, 0] + (V[i-1, 0] - V[i, 0]) * (Py - V[i, 1]) /
                                      (V[i-1, 1] - V[i, 1])))
                close = np.where(a, np.logical_not(close), close)

        # Penalty values
        safety = np.where(close, Kv / d, 0.0)

        # The penalty of each path is taken as the average penalty between its
        # inside and outside points
        safety_value += np.nanmean(safety, axis=1)
        safety_value
    return 100 - safety_value
...
def path_lenght(X, args):
    ...
    # safety_value = path_safety(obs, Px, Py)
    # Function to minimize
    #######################
    F = L * (1.0 + err) # + safety_value
    #######################
